...
CREATE TABLE Groups (
	...
	PostID INT,
	UserID INT,
    CONSTRAINT FK_Groups_Postings_Post FOREIGN KEY (PostID) REFERENCES Postings(PostID) ON DELETE CASCADE,
    CONSTRAINT FK_Groups_Users_User FOREIGN KEY (UserID) REFERENCES Users(UserID) ON DELETE CASCADE,
...
CREATE TABLE PostingTags (
	...
	PostID INT,
	TagID INT,
    CONSTRAINT FK_PostingTags_Postings_Post FOREIGN KEY (TagID) REFERENCES Tags(TagID) ON DELETE CASCADE,
    CONSTRAINT FK_PostingTags_Tags_Tag FOREIGN KEY (PostID) REFERENCES Postings(PostID) ON DELETE CASCADE,
	UNIQUE(PostID,TagID)
);
